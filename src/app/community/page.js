      ...
      <div className={`w-full bg-[#fcfcfc] ${!splashComplete ? "overflow-hidden h-screen" : ""}`}>
        ...
        <section ref={sectionRef} className="relative w-full flex flex-col pt-24 overflow-hidden">
          ...
          <div className="px-8 md:px-20 mb-16 max-w-5xl">
            ...
            <h2 className="text-[9vw] lg:text-[5vw] font-bold leading-[0.9] text-[#0F172A] uppercase" 
            ...
            {purposes.map((p) => {
              ...
              return (
                <div
                  ...
                  className={`relative rounded-[40px] p-8 md:p-10 cursor-pointer transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] flex-grow overflow-hidden
                    ${isActive 
                ...
                >
                  <div className="relative z-20 h-full flex flex-col justify-between">
                    ...
                    <div className={`transition-all duration-500 ${isActive ? 'lg:max-w-[50%]' : 'max-w-full'}`}>
                      <span className={`text-[10px] font-black tracking-widest uppercase mb-8 block transition-colors ${isActive ? 'text-[#df1612]' : 'text-gray-400'}`}>
                        Purpose 0{p.id}
                      </span>

                      <h3 className={`font-bold leading-[1.1] transition-all duration-500 uppercase italic ${isActive ? 'text-3xl md:text-4xl text-[#0F172A]' : 'text-xl text-gray-400'}`}
                          style={{ fontFamily: "'FoundersGrotesk', sans-serif" }}>
                        {p.title}
                      </h3>

                      <div className={`overflow-hidden transition-all duration-700 ease-in-out ${isActive ? 'max-h-96 opacity-100 mt-8' : 'max-h-0 opacity-0'}`}>
                        <div className="w-12 h-[2px] bg-[#df1612] mb-6" />
                        <p className="text-white md:text-gray-600 text-lg text-justify leading-relaxed font-medium">
                          {p.content}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className={`absolute top-0 right-0 h-full w-full lg:w-[50%] z-10 pointer-events-none transition-all duration-1000 ease-[cubic-bezier(0.25,1,0.5,1)]
          ...
          {/* JOIN COMMUNITY SECTION (Replaces the fixed bar to avoid disturbance) */}
          <div className="px-6 md:px-12 mt-20 mb-12">
             <div className="bg-[#0F172A] rounded-[48px] p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl overflow-hidden relative">
                {/* Visual Flair */}
                ...
                <div className="flex flex-col md:flex-row items-center gap-8 relative z-10 text-center md:text-left">
                    ...
                    <div className="max-w-md">
                        <h4 className="text-white font-bold text-3xl leading-tight uppercase mb-2" style={{ fontFamily: "'FoundersGrotesk', sans-serif" }}>
